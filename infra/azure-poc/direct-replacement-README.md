...
8. The USD 100 monthly budget retains 75/90/100 actual-cost alerts and current USD headroom covers the bounded maximum incremental replacement cost of USD 7.80.
9. The compiler, compiled parameter, and compiled template identities match the reviewed offline evidence.
10. Provider-level subscription what-if contains only Create/NoChange for the two replacement node sets (plus their bounded nested deployments), never Delete or a modification to an existing topology authority.
